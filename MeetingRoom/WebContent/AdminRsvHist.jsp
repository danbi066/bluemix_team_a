...
<body>
	...
	<div id="subtitle">특정 기간동안 회원들이 예약한 내용을 확인할 수 있는 페이지입니다.</div>
	...
	<div class="container">
		...
		<div align="right" style="margin-bottom: 20px;">
			<button type="button" class="btn btn-default" data-toggle="collapse"
				data-target="#delete">
				예약현황삭제<span class="glyphicon glyphicon-user-add"></span>
			</button>
		</div>

		<div id="delete" class="collapse" style="margin-bottom: 40px;"
			align="right">
			<form method="post" name="deleteForm" action="DeleteRsvRecord.do">
				<input type="hidden" name="page" value="AdminMember.jsp">
				<div class="form-inline">
					<input type="text" name="delete_date" id="delete_date"
						class="form-control"> 이전 날짜의 예약현황을 삭제합니다.
					<script>
					...
					<button type="button" onclick="dateCheckDelete();"
						class="btn btn-warning" style="margin-left: 10px;">삭제</button>
				</div>
			</form>
		</div>
		<br>
		...
		<form method="post" name="myForm" action="SearchHistory.do">
			<div class="form-inline">
				시작 날짜<input type="text" name="start_date" id="start_date"
					class="form-control"> 끝 날짜<input type="text"
				...
				<button type="button" onclick="dateCheckSearch();"
					class="btn btn-info">
		...
		<table class="table" style="margin-top: 30px;">
			...
			%>
			<tr>
				<td><%=dto.getHst_Rsv_Date()%></td>
				<td><%=time%></td>
				<td><%=dto.getHst_Rsv_Mem_Nm()%></td>
				<td><%=dto.getHst_Rsv_Mem_Pn()%></td>
				<td><%=dto.getHst_Rsv_Confer_Nm()%></td>
				<td><%=dto.getHst_Rsv_Title()%></td>
				<td><%=stat%></td>
				<td><%=dto.getHst_Date()%></td>
			</tr>
			<%
				}}
			%>
